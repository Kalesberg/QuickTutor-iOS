...
public class PinterestSegment: UIControl {
    ...
    private func reloadData() {
        ...
        var labelFullWidth: CGFloat = bounds.size.width / CGFloat(titles.count)
        if style.margin > 0 {
            labelFullWidth = (bounds.size.width - style.margin * 2) / CGFloat(titles.count)
        }
        ...
        for (index, title) in titles.enumerated() {
            ...
            if style.fitParent {
                titleX = style.margin + labelFullWidth * CGFloat(index) + (labelFullWidth - titleW) / 2
            } else {
