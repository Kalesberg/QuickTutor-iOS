...
extension RequestSessionBackgroundView: UITableViewDelegate, UITableViewDataSource {
	...
	func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		...
		updateTableViewCellOffset(height: Int(cell.bounds.height), row: indexPath.row)
	...
	func updateTableViewCellOffset(height: Int, row: Int) {
		if UIScreen.main.bounds.height <= 570 {
			DispatchQueue.main.async {
				if 1...4 ~= row {
					self.tableView.setContentOffset(CGPoint(x: 0, y: height * row), animated: true)
				}
			}
		} else {
			if 1...4 ~= row {
		...
		if datePickerIndexPath != nil && datePickerIndexPath!.row == indexPath.row {
			...
			headerView.backgroundColor = .black
				...
				case 0:
					...
					cell.datasource = self.requestData.subjects
					cell.delegate = self
					return cell
				case 1:
