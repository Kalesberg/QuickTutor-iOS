...
struct SpotlightTutor {
    ...
    public func queryFeaturedTutor(_ completion: @escaping (Error?) -> Void) {
        ...
        for category in Category.categories {
            ...
            ref.child("spotlight").queryOrdered(byChild: "t").queryEqual(toValue: categoryString).queryLimited(toFirst: 20).observeSingleEvent(of: .value) { snapshot in
    ...
    func queryByCategory(category: Category, _ completion: @escaping ([SpotlightTutor]?) -> Void) {
        ...
        ref.child("spotlight").queryOrdered(byChild: "t").queryEqual(toValue: category.mainPageData.displayName.lowercased()).queryLimited(toFirst: 50).observeSingleEvent(of: .value) { snapshot in
    ...
    func queryBySubject(subcategory: String, subject _: String, _: @escaping ([SpotlightTutor]?) -> Void) {
        var uids: [String]
        // we will need to create codes for every subject so that we can query a range of similar subjects...
        ref.child(subcategory).queryOrdered(byChild: "r").queryEqual(toValue: 5).queryLimited(toFirst: 50).observeSingleEvent(of: .value) { snapshot in
