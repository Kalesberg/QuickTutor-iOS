...
class EditTutorSubjects: BaseViewController {
    ...
    private func removeItem(item: Int) {
        ...
        contentView.pickedCollectionView.performBatchUpdates({
    ...
    private func deleteSubjects() {
        ...
        for i in 0 ..< subcategoriesToDelete.count {
            ...
            ref.child("subject").child(CurrentUser.shared.learner.uid!).child(subject).removeValue()
        }
        ...
        for subject in removedSubjects {
            ...
            ref.child("subcategory").child(subject.path.lowercased()).child(CurrentUser.shared.learner.uid!).child(formattedSubject).removeValue()
        }
    ...
    private func saveSubjects() {
        ...
        group.notify(queue: .main) {
            ...
            self.ref.updateChildValues(post) { _, _ in
    ...
    private func updateSubcategorySubjects(node: String, values: [String]?) {
        ...
        ref.child("subcategory").child(node).child(CurrentUser.shared.learner.uid!).updateChildValues(post)
    }
    ...
    override func handleNavigation() {
        if touchStartView is TutorPreferencesNextButton {
            ...
            if tableViewIsActive {
                ...
                if CurrentUser.shared.tutor.subjects != selectedSubjects {
                    backButtonAlert()
                } else {
...
extension EditTutorSubjects: UISearchBarDelegate {
    ...
    internal func searchBar(_: UISearchBar, textDidChange searchText: String) {
        ...
        if searchText == "" && !didSelectCategory {
            ...
            filteredSubjects = partialSubjects.filter({ $0.0.range(of: searchText, options: .caseInsensitive) != nil }).sorted(by: { $0.0.count < $1.0.count })
            contentView.tableView.reloadData()
        } else {
            tableView(shouldDisplay: true) {
                self.filteredSubjects = self.allSubjects.filter({ $0.0.range(of: searchText, options: .caseInsensitive) != nil }).sorted(by: { $0.0.count < $1.0.count })
                self.contentView.tableView.reloadData()
            }
        }
        scrollToTop()
