...
class EditPhoneVerificationVC: BaseViewController {
    ...
    @objc private func buildVerificationCode(_: UITextField) {
    ...
    private func updatePhoneNumber(_ credential: PhoneAuthCredential) {
        Auth.auth().currentUser?.updatePhoneNumber(credential, completion: { error in
            if let error = error {
                ...
                CurrentUser.shared.learner.phone = EditPhoneVerificationVC.phoneNumber
                ...
                self.navigationController?.popToViewController(LearnerEditProfileVC(), animated: true)
            }
        })
    }

    private func resendVCAction() {
        PhoneAuthProvider.provider().verifyPhoneNumber(EditPhoneVerificationVC.phoneNumber.cleanPhoneNumber(), uiDelegate: nil) { verificationId, error in
            if let error = error {
...
extension EditPhoneVerificationVC: UITextFieldDelegate {
