...
class RequestSessionPriceCell : UITableViewCell {
	...
	private func updateTextField(_ amount: String) {
		...
		currentPrice = this
		self.textField.attributedText = NSMutableAttributedString().bold("$\(number)", 32, .white).regular("/hr", 15, .white)
	...
	@objc func increasePrice() {
		...
		currentPrice += 1
		textField.attributedText = NSMutableAttributedString().bold("$\(self.currentPrice)", 32, .white).regular("/hr", 15, .white)
